#!/usr/bin/env python3
"""Batch calculation of stacking-fault energy (SFE) with LAMMPS.
...
DEFAULT_LAMMPS_IN = BASE_DIR / "calc_sfe.in.lmp"
RESULT_RE = re.compile(r"^RESULT_SFE,(.+)$", re.MULTILINE)
...
def main() -> None:
    ...
    parser.add_argument("--output", default=str(BASE_DIR / "results_SFE.csv"))
    parser.add_argument("--runs-dir", default=str(BASE_DIR / "runs_sfe"))
    ...
    with output.open("w", newline="", encoding="utf-8") as f:
        csv.writer(f).writerow(["Fe","Cr","Ni","Mn","SFE_mJm2","Eperfect_eV","Efault_eV","Area_A2","status","run_dir"])
            ...
            if not matches:
                ...
                if len(fields) == 8:
                    ...
                    try:
                        ...
                        status = "FAIL_PARSE_FLOAT"
                    record = fields + [status, run_dir.as_posix()]
                else:
